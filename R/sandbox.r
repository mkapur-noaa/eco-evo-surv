...
### Example of study question using gaussian random fields
...
#par(mfrow = c(2,2))
sim1 <- grf(1e3, grid = 'reg', 
cov.pars = c(0,0),
borders = matrix(c(5,0,0,5,0,0,5,5),ncol = 2, byrow = FALSE))

sim2 <- grf(1e3, grid = 'reg', 
cov.pars = c(0.5,0),
borders = matrix(c(5,0,0,5,0,0,5,5),ncol = 2, byrow = FALSE))

sim3 <- grf(1e3, grid = 'reg', 
cov.pars = c(5,0),##1,0
borders = matrix(c(5,0,0,5,0,0,5,5),ncol = 2, byrow = FALSE))

sim4 <- grf(1e3, grid = 'reg', 
cov.pars = c(5,1), ## 1,1
borders = matrix(c(5,0,0,5,0,0,5,5),ncol = 2, byrow = FALSE))
image(sim3)
...
stations <- sample(seq(length(sim1$data)),100) ## 100 random sampling sites

sim_sample <- subset(sims, id == 'sim1' & cellid %in% stations)['x.data']
