...
foreach(scenario=1:4) %:%
  ...
  foreach(repl=1:10)  %dopar%  {
    ...
    sp_use = species;
    replicate_use = repl;
    for(fc_use in c(0.15)){#0.15,0.15001)){
      # for(fc_use in c(1)){
...
strip_waa <- function(x){
  ...
  waa <- reshape2::melt(tmpwaa, id = c('year','rep','scenario','species'))
  names(waa)[5:6] <- c('age','weight_kg')
  waa$age <- as.numeric(waa$age)
  return(waa)
}
waa_index <- list.files(files_to_run,
  ...
  summarize(med=median(weight_kg),
...
ggplot(subset(acomps_index, year == 2040 & fc == 0.15),
  ...
  labs(x = 'Age', y = 'Frequency', color = '', fill = '')+
  theme(legend.position = 'none', axis.text.y = element_blank()) +
  facet_wrap(.~fc, labeller = as_labeller(fcLabs))

ggsave(last_plot(),
       file =here('figs',paste0('acompssimplesurvey-','atlanticherring','.png')),
       width = 3, height = 4, unit = 'in', dpi = 400)
